...
- `web_search_node`: Nút dự phòng, tự động tra cứu bổ sung dữ liệu nếu kho tài liệu nội bộ không đủ thông tin.
...
Scenario: Thực thi luồng Corrective RAG khi tài liệu nội bộ bị thiếu thông tin
  ...
  # Cạnh điều kiện 2 (Conditional Edge)
  Condition: Nếu tất cả tài liệu đều không đạt điểm chất lượng (< 0.7)
    Then: Đồ thị đi theo nhánh đến nút `web_search_node` để thu thập thêm thông tin văn bản hải quan mới.
    And: Sau khi có dữ liệu từ `web_search_node`, đồ thị quay lại nút `generate_node`.
